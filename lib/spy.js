...
const createSpyFactory = t => (fn = noop) => {
  ...
  const calledWith = (i, args, api = passed, name = '') => {
    const { pass } = t.eq(
      _args[i],
      args,
      `${name}should be called at #${i} with ${formatArgs(args)}`
    )
    return pass ? api : failed
  }
  ...
  const returned = (i, result, api = passed, name = '') => {
    const { pass } = t.eq(
      returns[i],
      result,
      `${name}call #${i} should return ${fastFormat(result)}`
    )
    return pass ? api : failed
  }
  ...
  spied.wasCalled = (...params) => {
    ...
    if (i == null || name !== '') {
      ...
      const msg =
        n > i
          ? `${name}was called ${times(cur)} before expected`
          : `${name}should have been called ${times(i + 1)}`
      r = t.eq(n, i + 1, msg)
    } else {
