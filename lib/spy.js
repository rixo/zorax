...
const sep = name => name && `spy:${name.trim()} `
...
const createSpyFactory = t => (fn = noop) => {
  ...
  const calledWith = (i, args, api = passed, name = '') => {
    const { pass } = t.eq(
      _args[i],
      args,
      `${sep(name)}should be called ${ordinal(i)} with ${formatArgs(args)}`
    )
    return pass ? api : failed
  }

  const returned = (i, result, api = passed, name = '') => {
    const { pass } = t.eq(
      returns[i],
      result,
      `${sep(name)}${ordinal(i)} call should return ${fastFormat(result)}`
    )
    return pass ? api : failed
  }
  ...
  spied.wasCalled = (...params) => {
    ...
    let i = index
    const name = typeof i === 'string' ? i + ' ' : ''
    let r = { pass: true }
    if (i == null || name !== '') {
      i = cur++
      const msg =
        n > i
          ? `${sep(name)}was called ${times(cur)} before expected`
          : `${sep(name)}should have been called ${times(i + 1)}`
      r = t.eq(n, i + 1, msg)
    } else {
  ...
  spied.wasNotCalled = msg => {
    t.ok(calls.length === cur, msg || 'should not have been called')
  }
