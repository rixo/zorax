...
const getHarnessMutators = pg => [pg.test, pg.proxy, pg.harness]
...
const getProxyMutators = pg => [pg.test, pg.proxy]

const getProxyDecorators = pg => [pg.decorate, pg.decorateProxy]
...
const createPlug = (ctx, parentPlugins, target, ...newPluginsInput) => {
  ...
  wrapTestMethod(ctx, plugins, proxy, getProxyHooks)
...
const wrapTestMethod = (harness, plugins, target, applyHooks) => {
  ...
  if (applyHooks) {
    const proxyHooks = applyHooks(plugins)
    proxyHooks.forEach(hook => hook(target, harness))
  }
}
